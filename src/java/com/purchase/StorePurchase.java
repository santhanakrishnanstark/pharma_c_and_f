...
@WebServlet(name = "StorePurchase", urlPatterns = {"/StorePurchase"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      boolean result=false;
        try{
            ...
            String invoice_no=request.getParameter("invno");
            String invoice_date=request.getParameter("invdate");
            ...
            int res = st.executeUpdate("insert into purchase(bill_date,invoice_no,invoice_date,supplier_no,sub_total,gst,discount,net_total) values ('"+bill_date+"','"+invoice_no+"','"+invoice_date+"','"+supplier_id+"','"+sub_total+"','"+gst+"','"+discount+"','"+net+"') ");
            ResultSet r = st.executeQuery("select max(bill_no) from purchase");
            if(r.next()){  bill_no = r.getInt(1); }
            System.out.println("Row : "+row+" Col: "+col);
            for(int i=0; i<=row; i++){
                StringBuilder query= new StringBuilder();
                query.append("insert into purchase_details values ");
                ...
                for(int j=0; j<=2; j++){
                    ...
                    if(j==2){ qtys = Integer.parseInt(records[j]); }
                }
                String pname=null;
                ...
                query.append("("+invoice_no+" ,"+id+",'"+pname+"',"+price+","+qtys+","+bill_no+") ");
                System.out.println(query.toString());
                int rss = st.executeUpdate(query.toString());
                int stockres = st.executeUpdate("INSERT INTO stock( entry_type, invoice_no, invoice_date, product_id, quantity) VALUES ('Purchase','"+invoice_no+"','"+invoice_date+"','"+id+"','"+qtys+"') ");
                if(rss >=0 && stockres >=0){
                    result=true;
                }
            }
